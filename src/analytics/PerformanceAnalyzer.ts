...
export class PerformanceAnalyzer {
  ...
  async analyze(userId: string, days = 7): Promise<SubScoreBreakdown> {
    const [allMissions, sleepLogs, activeGoals, recentProgress] = await Promise.all([
    ...
    // Split entry modes: live missions drive focus/estimation/completion sub-scores;
    // retroactive missions (the former habit logs) drive habit adherence.
    const liveMissions = allMissions.filter(m => m.mode === 'live');
    const completedLive = liveMissions.filter(m => m.status === 'completed');
    const retroMissions = allMissions.filter(m => m.mode === 'retroactive');

    // mission_consistency — distinct days with ≥1 completed live mission
    const completedDays = new Set(
      completedLive.map(m => new Date(m.completed_at!).toDateString())
    ).size;
    const missionConsistency = Math.round((completedDays / days) * 100);

    // completion_rate — completed / all terminal-status live missions
    const terminal = liveMissions.filter(m =>
      ['completed', 'failed', 'eta_expired'].includes(m.status)
    );
    const completionRate =
      terminal.length === 0
        ? 100
        : Math.round((completedLive.length / terminal.length) * 100);

    // estimation_accuracy — from EstimationAnalyzer (0-100), live missions only
    const estimation = analyzeEstimations(completedLive);
    ...
    // focus_duration — avg completed live mission duration normalised (90 min = 100)
    const durations = completedLive
    ...
    // habit_adherence — distinct days with ≥1 retroactive (habit) log
    const habitDays = new Set(
      retroMissions.map(m => new Date(m.completed_at ?? m.started_at).toDateString())
    ).size;
    ...
    // distraction_frequency — 100 minus proportion of failed/expired live starts
    const distracted = liveMissions.filter(m =>
      ['failed', 'eta_expired'].includes(m.status)
    ).length;
    const distractionFrequency =
      liveMissions.length === 0
        ? 100
        : Math.max(0, Math.round(100 - (distracted / liveMissions.length) * 100));
